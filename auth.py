...
class RegHandler(BaseHandler):
    ...
    def get(self):
        ...
        if self.get_argument("start_direct_auth", None):
            ...
            if not user["name"] or not user["room"]:
                ...
                + '<input class="form-control" type="text" name="room" placeholder="room" value="' + str(user["room"]) + '"> '
    ...
    def _reg(self, user):
        ...
        self.set_cookie("user_hash", urllib.quote( user["hash"] ))
        ...
        # check if string (key) 'room:user["room"]' exists or create new in list 'rooms'
        ...
        #create and push to list 'room:user["room"]' user name string 'user:user_name:user_hash'
        #create and push to object user as hash named 'user:user_name:user_hash'
        #try:
        ...
        #
        self.application.game.add_player( user["name"], user["hash"], 3)
...
class LogoutHandler(BaseHandler):
    def get(self):
        ...
        try:
            ...
            user_hash = urllib2.unquote( self.get_cookie("user_hash") )
        ...
        if room:
            room_str = 'room:'+room
            u_str = 'user:'+user_name+':'+user_hash
            self.application.client.lrem(room_str, u_str)
            
        
        
        self.clear_cookie('user_name')
        self.clear_cookie('user_hash')
